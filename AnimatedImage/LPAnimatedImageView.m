...
- (BOOL)shouldAnimate {
    ...
    return isShown && self.animatedImage != nil && _isPlaying && !_hasFinishedAnimating && self.animatedImage.frameCount > 1 && self.animatedImage.duration > 0;
}
...
- (void)timerFired:(CADisplayLink *)link {
    ...
    while (_elapseTime >= currentDelayTime) {
        ...
        if (_elapseTime == 0 && currentDelayTime == 0) { // 只有一贞时能跳出循环 否则 0 == 0
            break;
        }
    }
